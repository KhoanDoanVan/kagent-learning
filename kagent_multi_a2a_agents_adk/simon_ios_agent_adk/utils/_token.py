...
class KAgentTokenService:
  ...
  def event_hooks(self):
    """Returns a dictionary of event hooks for the application to use when creating the httpx.AsynClient"""
  ...
  async def _update_token_loop(self) -> None:
    ...
    # keep it updated - launch a background task to refresh it periodcally
  ...
  async def _refresh_token(self):
    while True:
      await asyncio.sleep(60) # Wait for 60s before refreshing
